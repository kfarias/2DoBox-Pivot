...
const buttons = {
 ...
 searchValue: function(){
   ...
   completeButton: function(){
     $(".idea-container").on("click", ".complete-task", (e) => {
       let selector = $(e.currentTarget).closest(".idea-card");
       selector.toggleClass('grey-background')
       let key = selector.attr("id");
       let ideaBox = storage.getIdea(key);
      //  funky shit happening below, toggling true/false for ideaBox.completed by setting it to the opposite of itself
       ideaBox.completed = !ideaBox.completed;
       storage.storeIdea(key, ideaBox);
     })
   }
